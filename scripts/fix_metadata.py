        ...
        if 'Contents' in page:
            ...
            if debug:
                if zipfile_count >= 200:
                    break
    zipfile_df = pd.DataFrame(np.array([zipfile_list,zipfile_sizes,contents_list,contents_objects], dtype=object).T, columns=['zipfile','size','contents','contents_object'])
    if debug:
        print(zipfile_df)
    return zipfile_df
