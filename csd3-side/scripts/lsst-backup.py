...
def compare_zip_contents_bool(collate_object_names, current_objects: pd.DataFrame, destination_dir: str) -> bool:
    ...
    # print(f'collate_object_names type: {type(collate_object_names)}', flush=True)
    ...
    if not current_objects.empty:
        ...
        if isin.any():
            ...
            dprint(f'Zip file does not exist - uploading.', flush=True)
...
if __name__ == '__main__':
    ...
    with Client(n_workers=n_workers,threads_per_worker=threads_per_worker,memory_limit=mem_per_worker) as client:
        ...
        # quick check delete
        print(sorted(current_objects[current_objects['CURRENT_OBJECTS'].str.contains('collated_') == True]['CURRENT_OBJECTS'].apply(lambda x: x.split('_')[1].split('.')[0])).to_list()[-50:])
        exit()
