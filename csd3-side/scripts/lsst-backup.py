...
def process_files(
    ...
    print(f'type(targets) after compute: {type(targets_df)}', flush=True)
    targets_df.to_csv('test_targets_after_compute.csv', index=False)
    targets_df = targets.reset_index(drop=True)
    print(targets_df, flush=True)
    print(type(targets_df), flush=True)
    # Add symlink target paths to ddf
    ddf = ddf.compute()
    ddf = pd.concat([ddf, targets_df])
    del targets_df
