                    ...
                    if result.ready():
                        ...
                        if (parent_folder,id) in uploaded: # this seems wasteful - zipping has occurred before this point
                            ...
                            if current_objects.isin([to_collate[parent_folder]['zips'][-1]['zip_object_name']]).any():
                                ...
                                try:
                                    existing_zip_contents = bm.get_resource(access_key, secret_key, s3_host).Object(bucket_name,''.join([to_collate[parent_folder]['zips'][-1]['zip_object_name'],'.metadata'])).get()['Body'].split(',')
